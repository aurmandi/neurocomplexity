...
def wilting_mr(rec: SpikeRecording,
    ...
    _warn_if_uncurated(rec, "wilting_mr")
...
def branching_ratio(rec: SpikeRecording,
    ...
    Thin public entry point that accepts ``bin_size`` in seconds (rather than
    ``bin_size_ms`` in milliseconds used by the lower-level :func:`wilting_mr`).
    """
    from neurocomplexity._warnings import _warn_if_uncurated
    ...
    bs = float(bin_size)
    _params = {"populations": list(populations), "bin_size": float(bin_size),
               "k_max": int(k_max), "k_min": int(k_min)}
    A = bin_all_active(rec, populations, bs).astype(np.float64)
    T = A.size
    if T < k_max + 10:
